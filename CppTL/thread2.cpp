        ...
        for (;;) 
        {
         ...
         //  }
            ...
            //}  
                ...
                m3.unlock();
                m4.unlock();
                while (B.flag == 1);//等到button执行完毕，再执行SN方向的绿灯。
                GRLight(&CSN);
                //if (GRLight(&CSN) == 5)
                //{
                    // m1.unlock();
               ...
               // {
                ...
                if (WLSN.CheckB() == -1&& YellowLight(&CSN) == 0)//else// car.lock();
                {
                    ...
                    button2.unlock();//car1.unlock();//对应的  car1.lock() 在构造函数中；
                }//只有一个获得了锁。 
                //car1.lock();
                m1.lock();
                m2.lock();
